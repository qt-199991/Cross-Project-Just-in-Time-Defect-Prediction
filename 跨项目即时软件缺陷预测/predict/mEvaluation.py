...
if __name__ == '__main__':
    ...
    a = 3
    b = 5
    lsts = [[[] for _ in range(b)] for _ in range(a)]
    print(lsts)
    print(np.shape(lsts))
    lsts[2][2] = [2,1,3]
    print(lsts)

    # evaluate(mydata.getPred()[23:24], mydata.getY()[23:24])

    # result = evaluate(mydata.getPred(), mydata.getY())
    # result = pd.DataFrame(
    #     {'Evaluate':['Accuracy','Precision','Recall','F1-Score','hitrate','subsetAcc','hamming_loss','oneerror','coverage','ranking_loss','average_precision'],
    #     'Score':result} )
    # print(result)